...
memtypes =[types.StringType, types.IntType, types.StringType, types.StringType,
		   types.StringType, types.NoneType, types.IntType, types.IntType,
...
class PgConnectionTestCases(unittest.TestCase):
	...
	def CheckMemberValues(self, cnx, expected):
		...
		for elt in self.members:
			print "%s: %s" %(elt, getattr(cnx,elt))
		for elt, val in zip(self.members, expected):
			v = (getattr(cnx,elt) == val)
	...
	def CheckMemberTypes(self, cnx, expected):
		for elt in self.members:
			print "%s: %s, %s" %(elt, type(getattr(cnx,elt)) ,getattr(cnx,elt) )
		for elt, val in zip(self.members, expected):
			v = getattr(cnx,elt)
			self.assertEquals(type(v), val,
	...
	Test sendQuery, consumeInput, and getResult.
		"""
		#UNDONE - update for versions 8.0 ->9.0?
		...
		try:
			...
			self.assertEquals(res.nfields, flen,
							  'Query return wrong number of fields (Varies by server version).')
	...
	Check the object for valid member types and values.
		...
		# WARNING. EVIL. access has side effects.
		# UNDONE Deprecate.
		# WARNING - This segfaults in test when run in the original method.
		self.assertEquals(cnx.toggleShowQuery, 'text',
						  'Turning on ShowQuery failed.')
		self.assertEquals(cnx.toggleShowQuery, None,
						  'Turning off ShowQuery failed.')

		#print "turning toggleShowQuery on"
		val = cnx.toggleShowQuery
		#print val
		self.assertEquals(val, 'text', 'Turning on ShowQuery failed.')
		#print "done turning on"
		#print "turning toggleShowQuery off"
		val = cnx.toggleShowQuery
		#print val
		self.assertEquals(val, None, 'Turning off ShowQuery failed.')

class PgConnectionCloseTestCases(PgConnectionTestCases):

	def CheckFinish(self):
		"""
	Test finish()
		"""
		# Bad test case, no cookie. pass fail depends on this one running last.
		try:
			print "Finish"
			cnx.finish()
			expected = [None, None, None, None, None, None, None, None, None,
...
if __name__ == "__main__":
	...
	# Bad test case, no cookie. pass fail depends on this one running last.
	TestSuite = unittest.TestSuite()
	TestSuite.addTest(PgConnectionCloseTestCases("CheckFinish"))
	runner = unittest.TextTestRunner()
	runner.run(TestSuite)
